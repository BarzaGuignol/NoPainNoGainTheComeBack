...
class VehicleManager extends AbstractManager {
  ...
  findDate(departure_date, arrival_date, type) {
    let query = `SELECT v.id, v.model, v.kilometer, v.picture, v.type, v.vehicle_year
    FROM vehicle v
    ...
    const value = [departure_date, arrival_date];
    if (type) {
      query += `AND v.type = ?`;
      value.push(type);
    }
